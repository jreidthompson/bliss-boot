...
class Scanner(object):
		...
		def find_kernels(self):
				...
				if not os.path.exists(conf.bootdir):
						...
						if not os.path.exists(conf.bootdir):
								toolkit.die("Kernel boot directory doesn't exist")
						else:
				...
				if out:
						for i in out:
								self.__kernels.append(i.strip())
				else:
						toolkit.die("No kernels in boot directory")
		...
		def scan_fstab(self):
				...
				if out:
						...
						toolkit.die("/boot could not be found in /etc/fstab")
		...
		def detect_layout(self):
				...
				if temp:
						...
						if out:
								...
								toolkit.die("Partition Layout could not be " + 
								"detected for: " + temp)
				else:
						toolkit.die("Could not find the value assigned to " + 
						"this boot drive.")
		...
		def get_bootdrive(self):
				...
				if conf.zfs == 1:
						...
						toolkit.die("Could not parse the 'zfs_boot' variable " +
						"correctly.")
						...
						if match:
								...
								if m1:
										...
										if alph:
												...
												alphindex = toolkit.get_alph_index(alph.group(0))
								...
								# explictly set it
								toolkit.die("Could not generate the appropriate " +
